...
last_reply_time = defaultdict(lambda: datetime.min)
...
async def safe_reply(event, message):
    try:
        await event.respond(message, comment_to=event.id)
        return True
    except FloodWaitError as e:
        logger.warning(f"Flood wait {e.seconds} сек, спим...")
        await asyncio.sleep(e.seconds)
        return False
    except ChatWriteForbiddenError:
        logger.warning(f"Нет прав на отправку в чате {event.chat_id}")
        return False
...
async def handler(event):
    try:
        ...
        now = datetime.now()
        if now - last_reply_time[chat_id] < timedelta(seconds=COOLDOWN_SECONDS):
            return
...
if __name__ == '__main__':
    try:
        ...
        logger.info("Бот остановлен")        "Унікальна футболка з козаком – подарунок для себе чи друга. 20% на ЗСУ. {url}",
        "Долучайся до допомоги – купуй футболку з українським козаком. {url}",
        "Мерч, який рятує. 20% від кожної футболки йде на гуманітарку. {url}",
        "Футболка з історією. Козак – символ незламності. Замовити: {url}",
        "Зроби свій внесок у перемогу – обери футболку з козаком. {url}",
...
# Хранилище времени последнего ответа в каждом чате
last_reply_time = defaultdict(lambda: datetime.min)
...
async def safe_reply(event, message):
    """Безопасно отвечает с обработкой flood wait и ошибок прав."""
    try:
        await event.respond(message, comment_to=event.id)
        return True
    except FloodWaitError as e:
...
async def handler(event):
    try:
        ...
        # Проверяем, не слишком ли часто отвечаем в этом чате
        chat_id = event.chat_id
        now = datetime.now()
        if now - last_reply_time[chat_id] < timedelta(seconds=COOLDOWN_SECONDS):
            return
        ...
        # Если сообщение является ответом на пост
        if event.is_reply:
            original_post = await event.get_reply_message()
            if original_post:
                original_text = original_post.text or ''
